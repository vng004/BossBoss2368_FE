...
const ProductHot = () => {
  const { formatPrice, listProducts } = useContext(ProductContext) as ProductContextType;

  const productsHot = listProducts.filter(p => p.hot).slice(0, 5)

  return (
    <div className="w-full space-y-2 mt-3">
      <div className="text-lg flex gap-2 border-b pb-3"><p>Sản phẩm bán chạy</p> <Flame className="text-[#ef4d38]" /></div>
      <div className='flex flex-col gap-4 '>
        {productsHot.map(product => {
          const discountPrice = formatPrice(Number(product.colors[0].discountPrice))
                  ...
                  <div>
                    {discountPrice !== price ? (
                      <div className="flex flex-col">
                        ...
                        <span className="font-semibold text-[#ef4d38]">{discountPrice}</span>
                      </div>
                    ) : (
                      <p className="font-semibold text-[#ef4d38]">{discountPrice}</p>
                    )}
                  </div>
                </div>
              </div>
